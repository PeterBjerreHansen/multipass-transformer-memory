...
The command shows the active periodic Bank and adaptive Recirculation arms as
...
uv run python benchmarks/development/stage_4_confirmation/prepare.py \
  --fast recirculation_adaptive --bank periodic32 --hybrid recirculation
...
`--fast` accepts only `recirculation_adaptive`; `--bank` accepts `dense`,
