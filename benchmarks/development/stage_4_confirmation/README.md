...
uv run python benchmarks/development/stage_4_confirmation/prepare.py \
  --fast recirculation_adaptive --memory-attention strided32 --hybrid recirculation
...
`--fast` accepts only `recirculation_adaptive`; `--memory-attention` accepts
`dense`, `strided32`, or `memory_token32`; `--hybrid` accepts `recirculation` and
