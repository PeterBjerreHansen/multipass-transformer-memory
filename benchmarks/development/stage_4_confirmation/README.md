...
The command shows the active periodic Tape and adaptive Recirculation arms as
...
uv run python benchmarks/development/stage_4_confirmation/prepare.py \
  --fast recirculation_adaptive --tape periodic32 --hybrid recirculation
...
`--fast` accepts only `recirculation_adaptive`; `--tape` accepts `dense`,
